...
4. **MediaPipe server** (optional Python process via Unix socket `/tmp/pt_turtle.sock`)
...
- **MediaPipeClient** (`Core/MediaPipeClient.swift`): IPC with Python server via Unix Domain Socket. Optional enhancement — app works fully without it.

### Python Components

`python_server/pose_server.py`: MediaPipe Tasks API server. Listens on Unix socket. Provides 478-landmark face mesh and body pose as alternative detection source.
...
|-----------|---------------|-----------|
| **Code review** | `codex-collab review` | Reads all files, no Claude tokens spent |
| **Multi-file refactoring** | `codex-collab run "refactor X across Y"` | Edits many files without Claude context |
| **New feature implementation** | `codex-collab run "implement X"` | Full implementation without Claude reading/writing |
...
### When Claude Should Act Directly (token-cheap)
...
### Workflow Pattern: Claude Orchestrates, Codex Executes

1. **User requests feature** → Claude plans the approach (cheap: text only)
2. **Implementation** → Claude delegates to Codex via `codex-collab run` (background)
3. **While Codex works** → Claude can handle other user questions or launch parallel Codex tasks
4. **Codex completes** → Claude reviews output summary, runs `./build.sh` to verify
5. **Issues found** → Claude resumes Codex thread: `codex-collab run --resume <id> "fix the build error: ..."`
6. **Done** → Claude reports to user
...
| Action | Claude-only cost | With Codex delegation |
|--------|------------------|-----------------------|
